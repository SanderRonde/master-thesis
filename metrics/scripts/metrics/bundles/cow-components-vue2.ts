...
const BASE_DIR = path.join(METRICS_DIR, `collectors/cow-components-vue2`);

export const cowComponentsVue2Metrics = preserveCommandBuilder(
	cmd('cow-components-vue2-metrics')
		.desc('Collect cow-components-vue2 metrics')
		.args({
			'no-cache': flag(),
			prod: flag(),
		})
